...
class CameraApp(QMainWindow):
    ...
    def update_frame(self):
        ...
        if (__PC__ == True):
            frame = np.random.randint(0, 200, size=(X_SIZE_IMAGE_RAW, Y_SIZE_IMAGE_RAW), dtype=np.uint8)
        ...
        if (result_proc["contrast"] != None):
            self.Contrast_draw_label.setText("{0:3.3f}".format(result_proc["contrast"]))
        ...
        self.qimage2 = QImage(result_proc["histogram"], 384, 192, 384, QImage.Format_RGB888)
        # display images
